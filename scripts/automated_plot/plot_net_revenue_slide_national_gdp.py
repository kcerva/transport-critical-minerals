"""
Net Export Revenue - National Policy Scenarios with GDP Share

Two-panel visualization:
- Left: Stacked horizontal bars showing total net export revenue by country
- Right: Dot plot showing GDP share for top countries across scenarios

Focuses on National (country) policy scenarios: Baseline, BAU, Early Refining

Output: net_export_revenue_slide_national_gdp.png
"""
...
def load_gdp_data():
    ...
    # GDP data comes from all_data.xlsx
    all_data_path = results_path / 'all_data.xlsx'

    print(f"Loading GDP data from: {all_data_path}")
    df = pd.read_excel(all_data_path)

    # Get 2022 GDP (from baseline scenario)
    df_2022 = df[df['scenario'] == '2022_baseline']
    gdp_2022 = df_2022[df_2022['gdp_usd'] > 0].groupby('iso3')['gdp_usd'].first().to_dict()

    # Get 2040 GDP (already projected in the data for 2040 scenarios)
    # Use any 2040 scenario - they all have the same projected GDP
    df_2040 = df[df['scenario'].str.contains('2040')]
    gdp_2040 = df_2040[df_2040['gdp_usd'] > 0].groupby('iso3')['gdp_usd'].first().to_dict()

    print(f"  Loaded GDP data for {len(gdp_2022)} countries")
    print(f"  Sample: COD 2022=${gdp_2022.get('COD', 0)/1e9:.1f}B, 2040=${gdp_2040.get('COD', 0)/1e9:.1f}B")

    return gdp_2022, gdp_2040
...
def generate_dual_panel_chart(output_dir):
    """Generate dual-panel chart: revenue bars + GDP dot plot."""
    os.makedirs(output_dir, exist_ok=True)

    print("\n[Net Export Revenue - National Policy with GDP Share]")
    print("-" * 70)

    # Load data
    df_flows = load_tonnage_flows_data()
    gdp_2022, gdp_2040 = load_gdp_data()

    # Define scenarios (National policy only)
    scenarios_config = [
        ('2022_baseline', 'country_unconstrained', 'Baseline', gdp_2022),
        ('bau_2040_mid_min_threshold_metal_tons', 'country_unconstrained', 'BAU', gdp_2040),
        ('early_refining_2040_mid_min_threshold_metal_tons', 'country_unconstrained', 'Early Refining', gdp_2040),
    ]
    ...
    # Determine which countries to show in GDP panel (union of all top 5s)
    # Sort by max GDP share across scenarios for better ordering
    gdp_panel_countries = []
    for country in all_top_countries_list:
        max_gdp = max(s['gdp_shares'].get(country, 0) for s in scenario_data)
        gdp_panel_countries.append((country, max_gdp))
    ...
    fig, (ax_bars, ax_dots) = plt.subplots(1, 2, figsize=(12, 5.5),
    ...
    for i, data in enumerate(scenario_data):
        ...
        for country, value in countries_sorted:
            ...
            # Add ISO3 label inside if segment is wide enough (minimum $12B width)
            if value > 12:
    ...
    ax_bars.set_title('Total Net Export Revenue\n(National Policy)', fontsize=12, fontweight='bold')
    ...
    # ========== RIGHT PANEL: GDP share grouped horizontal bars (BAU + Early Refining only) ==========
    # Filter to only BAU and Early Refining scenarios
    gdp_scenarios = [s for s in scenario_data if s['label'] in ['BAU', 'Early Refining']]
    ...
    for i, country in enumerate(gdp_panel_countries):
        for j, data in enumerate(gdp_scenarios):
            gdp_share = data['gdp_shares'].get(country, 0)
            label = data['label']

            # Offset bars vertically (BAU on top, Early Refining on bottom)
            y_offset = bar_height/2 if j == 0 else -bar_height/2

            ax_dots.barh(i + y_offset, gdp_share, height=bar_height,
                        color=gdp_bar_colors[label], edgecolor='white', linewidth=0.5)

            # Add value label at end of bar if value > 0
            if gdp_share > 0:
                # Smaller font for small values
                fontsize = 8 if gdp_share < 10 else 9
                ax_dots.text(gdp_share + 1, i + y_offset, f'{gdp_share:.0f}%',
                            va='center', ha='left', fontsize=fontsize,
                            color=gdp_bar_colors[label], fontweight='bold')

    ax_dots.set_yticks(y_positions_bars)
    ax_dots.set_yticklabels([COUNTRY_NAMES.get(c, c) for c in gdp_panel_countries], fontsize=11)
    ax_dots.set_xlabel('Net Export Revenue as Share of GDP (%)', fontsize=11, fontweight='bold')
    ax_dots.set_title('GDP Share by Country\n(National Policy)', fontsize=12, fontweight='bold')

    # Set x-axis limits
    max_gdp = max(max(s['gdp_shares'].values()) for s in gdp_scenarios if s['gdp_shares'])
    ax_dots.set_xlim(0, max_gdp * 1.15)

    ax_dots.xaxis.grid(True, linestyle='--', alpha=0.3, zorder=0)
    ax_dots.yaxis.grid(False)
    ...
    from matplotlib.patches import Patch
    scenario_legend = [
        Patch(facecolor=gdp_bar_colors['BAU'], edgecolor='white', label='BAU'),
        Patch(facecolor=gdp_bar_colors['Early Refining'], edgecolor='white', label='Early Refining'),
    ]
    ax_dots.legend(handles=scenario_legend, loc='upper right', fontsize=9,
                   frameon=True, fancybox=True)

    plt.tight_layout()

    # Save
    output_path = os.path.join(output_dir, 'net_export_revenue_slide_national_gdp.png')
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("NET EXPORT REVENUE - NATIONAL POLICY WITH GDP SHARE")
    print("=" * 70)
