"""
Infrastructure Indicators Figure
...
- Row 2: By Country (Transport Volume | Energy Capacity)
...
def prepare_infrastructure_data(df):
    """
    Prepare data for all 6 infrastructure panels

    Returns:
        tuple: Six dictionaries for each panel
    """
    ...
    for goal, policy, constraint, label in SCENARIO_CONFIG:
        ...
        energy_country_data[label] = {'low': {}, 'mid': {}, 'high': {}}
        ...
        if goal == 'baseline':
            ...
            for demand in ['low', 'mid', 'high']:
                ...
                # Energy by country (top 8 + Other)
                by_country_energy = df_scenario.groupby('iso3')['energy_req_capacity_kW'].sum()
                for country in top_energy_countries:
                    energy_country_data[label][demand][country] = by_country_energy.get(country, 0) / 1e6
                other_energy = by_country_energy[~by_country_energy.index.isin(top_energy_countries)].sum()
                energy_country_data[label][demand]['Other'] = other_energy / 1e6
    ...
    return (transport_mineral_data, energy_mineral_data,
            ...
            top_transport_countries, top_energy_countries, country_colormap)
...
def generate_infrastructure_indicators_figures(df, output_dir):
    """
    Main entry point for generating infrastructure indicators figure
    ...
    """
    print("  Generating infrastructure indicators 6-panel figure...")
    ...
    country_order_energy = top_energy_countries + ['Other']
    country_colors_energy = {country: country_colormap.get(country, '#999999') for country in top_energy_countries}
    country_colors_energy['Other'] = '#999999'
    ...
    plot_stacked_panel(
        ax_energy_country, energy_country_data,
        stack_order=country_order_energy,
        colors=country_colors_energy,
        title='E) Installed Electricity Capacity by Country',
        xlabel='Capacity (GW)'
    )
    ...
    # Country legends (top 8 + Other) - Note: countries may differ between transport and energy
    # For simplicity, we'll show transport countries on left, energy countries on right
    ...
    energy_country_patches = []
    for country in (top_energy_countries + ['Other']):
        color = country_colors_energy[country]
        patch = mpatches.Patch(facecolor=color, label=country, edgecolor='black', linewidth=0.8)
        energy_country_patches.append(patch)

    legend_energy_countries = ax_energy_country.legend(
        handles=energy_country_patches,
        title='Countries (Energy)',
        bbox_to_anchor=(1.02, 0.40),  # Below constraint legend
        loc='upper left',
        fontsize=7,
        title_fontsize=8,
        framealpha=0.98,
    ...
    ax_energy_country.add_artist(legend_energy_countries)
    ...
    legend_constraint = ax_energy_country.legend(handles=constraint_patches + [error_patch],
                                                 title='Constraint &\nUncertainty',
                                                 bbox_to_anchor=(1.02, 1),
                                                 loc='upper left',
                                                 fontsize=8,
    ...
    ax_energy_country.add_artist(legend_constraint)
    ...
    png_path = os.path.join(output_dir, 'infrastructure_indicators_six_panel.png')
    plt.savefig(png_path, dpi=DPI_PUBLICATION, bbox_inches='tight', facecolor='white')
    saved_paths.append(png_path)
    print(f"    ✓ Saved: {os.path.basename(png_path)}")

    pdf_path = os.path.join(output_dir, 'infrastructure_indicators_six_panel.pdf')
    plt.savefig(pdf_path, format='pdf', bbox_inches='tight', facecolor='white')
    saved_paths.append(pdf_path)
    print(f"    ✓ Saved: {os.path.basename(pdf_path)}")

    preview_path = os.path.join(output_dir, 'infrastructure_indicators_six_panel_preview.png')
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(config['paths']['figures'], 'automated_plots', 'publication', 'infrastructure_indicators')
    os.makedirs(output_dir, exist_ok=True)

    # Generate figure
    print("Testing infrastructure indicators figure generation...")
    paths = generate_infrastructure_indicators_figures(df, output_dir)
