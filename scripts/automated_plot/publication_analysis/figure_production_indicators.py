"""
BAU vs Precursor Product Comparison Figure

Creates a five-panel figure comparing Business as Usual (BAU) and Precursor Product
scenarios with demand uncertainty ranges:

Panel B: Total extraction by mineral (stage 0 - metal content)
Panel C: Total processing by mineral (stage > 0 - Copper, Manganese, Graphite, Cobalt, Nickel, Lithium)
Panel D: Total processing by type (stage > 0 - Beneficiation, Early refining, Precursor)
Panel E: Extraction by country (stage 0 - metal content)
Panel F: Processing by country (stage > 0 - all processing types)
...
def prepare_processing_data(df):
    """
    Prepare processing data (stage > 0) with 7 bars distinguishing BAU vs Precursor
    ...
    Returns:
        tuple: (panel_processing_data, panel_mineral_data)
            panel_processing_data: Dict[scenario_label][demand][processing_type] = production (stage > 0)
            panel_mineral_data: Dict[scenario_label][demand][mineral] = production (stage > 0)
    """
    # Panels for processing (stage > 0)
        ...
        if goal == 'baseline':
            ...
            for demand in ['low', 'mid', 'high']:
                ...
                if not df_scenario_processing.empty:
                    by_processing = df_scenario_processing.groupby('processing_type')['production_tonnes'].sum()
                    ...
                    by_mineral_processing = df_scenario_processing.groupby('reference_mineral')['production_tonnes'].sum()
                    for mineral in MINERAL_ORDER:
...
def prepare_country_processing_data(df):
    """
    Prepare country-level processing data (stage > 0) with 7 bars
    ...
    Returns:
        tuple: (country_processing_data, top_countries)
            country_processing_data: Dict[scenario_label][demand][country] = production
            top_countries: List of top 8 country ISO codes
    """
    df_processing = df[df['processing_stage'] > 0].copy()

    # Get top 8 countries by total processing
    top_countries = get_top_countries(df_processing, 'production_tonnes', n=8)
...
def plot_stacked_panel(ax, data, stack_order, colors, title, xlabel='Production (Mt)', scenario_config=None):
    """
    ...
    ax.set_xlim(0, 38)  # Slightly above max for error bars
...
def create_production_indicators_figure(df, output_dir):
    """
    Generate complete production indicators comparison figure
    ...
    """
    print("  Generating production indicators three-panel figure...")
    ...
    # Prepare data
    panel_metal_data = prepare_extraction_data(df)
    panel_processing_data, panel_mineral_data = prepare_processing_data(df)
    country_extraction_data, top_extraction_countries = prepare_country_extraction_data(df)
    country_processing_data, top_processing_countries = prepare_country_processing_data(df)
    ...
    plot_stacked_panel(
        ...
        title='C) Total Processing by Mineral',
        xlabel='Production (Mt)',
        scenario_config=SCENARIO_CONFIG_PROCESSING
    )
    ...
    plot_stacked_panel(
        ...
        title='D) Total Processing by Type',
        xlabel='Production (Mt)',
        scenario_config=SCENARIO_CONFIG_PROCESSING
    )
    ...
    png_path = os.path.join(output_dir, 'production_indicators_three_panel.png')
    ...
    pdf_path = os.path.join(output_dir, 'production_indicators_three_panel.pdf')
    ...
    preview_path = os.path.join(output_dir, 'production_indicators_three_panel_preview.png')
...
def generate_production_indicators_figures(df, output_dir):
    """
    Main entry point for generating production indicators figures
    ...
    Returns:
        List of saved file paths
    """
    return create_production_indicators_figure(df, output_dir)
...
if __name__ == '__main__':
    ...
    print("Testing production indicators figure generation...")
    paths = generate_production_indicators_figures(df, output_dir)
