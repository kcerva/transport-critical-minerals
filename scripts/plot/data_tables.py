...
# Extract the policy base for mapping
def pct_change_between(df, constraint_1, constraint_2, value_cols):

    def shorten_scenario_name(s):
        return s.replace('_threshold_metal_tons', '')
    ...
    for base_tag, comp_tag in POLICY_MATCHES.items():
        for year in sorted(df1['year'].dropna().unique()):
            y1 = year
            y2 = str(int(year) + 10)  # e.g., 2030 → 2040

            r1 = df1[(df1['year'] == y1) & (df1['tag'] == base_tag)]
            r2 = df2[(df2['year'] == y2) & (df2['tag'] == comp_tag)]

            # print(f"\n🔎 Comparing {y1}_{base_tag} (country) → {y2}_{comp_tag} (region)")
            # print(f" - country rows: {len(r1)}, region rows: {len(r2)}")

            if not r1.empty and not r2.empty:
                for _, row1 in r1.iterrows():
                    for _, row2 in r2.iterrows():
                        result = {
                            'scenario': f"{shorten_scenario_name(row1['scenario'])}_vs_{shorten_scenario_name(row2['scenario'])}",
                            'constraint': f"percentage_change_{constraint_1.split('_')[0]}"
                        }
                        for col in value_cols:
                            val1 = row1.get(col, None)
                            val2 = row2.get(col, None)
                            if pd.notna(val1) and pd.notna(val2) and val1 != 0:
                                pct = ((val2 / val1) * 100) - 100
                            else:
                                pct = None
                            result[col] = pct
                            # print(f"    {col}: {val1} → {val2} → % change = {pct}")
                        rows.append(result)
...
def create_pivot_with_pct_change(df, value_column, unit_label, conversion_factor=1.0, to_kt=False):
    ...
    pct_constrained = pct_change_between(pivot, 'country_constrained', 'region_constrained', value_cols)
    pct_unconstrained = pct_change_between(pivot, 'country_unconstrained', 'region_unconstrained', value_cols)
...
def create_metal_content_table(df, to_kt=False):
    ...
    try:
        pct_c = pct_change_between(pivot_reset, 'country_constrained', 'region_constrained', value_cols)
        pct_u = pct_change_between(pivot_reset, 'country_unconstrained', 'region_unconstrained', value_cols)
...
def create_unit_cost_table(df):
    ...
    try:
        pct_c = pct_change_between(combined, 'country_constrained', 'region_constrained', value_cols)
        pct_u = pct_change_between(combined, 'country_unconstrained', 'region_unconstrained', value_cols)
...
def create_revenue_tables(df, to_kt=False):
    ...
    main['pct_change_constrained'] = ((main.get('region_constrained', 0) / main.get('country_constrained', 1)) * 100) - 100
    main['pct_change_unconstrained'] = ((main.get('region_unconstrained', 0) / main.get('country_unconstrained', 1)) * 100) - 100
...
def create_summary_mid_demand_unconstrained(df):
    # Filter to relevant constraints and scenario types
    df_filtered = df[
    ...
    summary = df_filtered.groupby(['year', 'constraint']).agg({
        'production_tonnes': lambda x: x.sum() / 1e6,     # Mt
        'all_cost_usd': lambda x: x.sum() / 1e6,          # USD million
        'revenue_usd': lambda x: x.sum() / 1e6            # USD million
    }).reset_index()

    # Pivot the table to wide format
    summary_pivot = summary.pivot(index='year', columns='constraint')

    # Flatten column MultiIndex
    summary_pivot.columns = [f"{metric}_{constraint}" for metric, constraint in summary_pivot.columns]
    summary_pivot = summary_pivot.reset_index()

    # Calculate % changes
    def percent_change(c_region, c_country):
        return ((c_region / c_country) * 100 - 100) if c_country != 0 else None

    summary_pivot['production_pct_change'] = summary_pivot.apply(
        lambda row: percent_change(row.get('production_tonnes_region_unconstrained', 0),
                                   row.get('production_tonnes_country_unconstrained', 0)), axis=1)
    summary_pivot['cost_pct_change'] = summary_pivot.apply(
        lambda row: percent_change(row.get('all_cost_usd_region_unconstrained', 0),
                                   row.get('all_cost_usd_country_unconstrained', 0)), axis=1)
    summary_pivot['revenue_pct_change'] = summary_pivot.apply(
        lambda row: percent_change(row.get('revenue_usd_region_unconstrained', 0),
                                   row.get('revenue_usd_country_unconstrained', 0)), axis=1)

    # Reorder and rename for output clarity
    summary_table = summary_pivot[[
        'year',
        'production_tonnes_country_unconstrained',
        'production_tonnes_region_unconstrained',
        'production_pct_change',
        'all_cost_usd_country_unconstrained',
        'all_cost_usd_region_unconstrained',
        'cost_pct_change',
        'revenue_usd_country_unconstrained',
        'revenue_usd_region_unconstrained',
        'revenue_pct_change'
    ]]

    summary_table.columns = [
        "Year",
        "Production Vol (Country, Mt)",
        "Production Vol (Region, Mt)",
        "Production % Change",
        "Total Cost (Country, MUSD)",
        "Total Cost (Region, MUSD)",
        "Cost % Change",
        "Revenue (Country, MUSD)",
        "Revenue (Region, MUSD)",
        "Revenue % Change"
    ]
...
def generate_pivot_excel_files(df: pd.DataFrame, global_output_path: str, country_output_folder: str):
    ...
    with ExcelWriter(global_output_path, engine='openpyxl') as writer:
        ...
        # Summary mid-demand unconstrained tab
    ...
    for iso3 in df['iso3'].dropna().unique():
        ...
        try:
            with ExcelWriter(country_file_path, engine='openpyxl') as writer:
                ...
                # Summary mid-demand unconstrained tab for country
