"""Mine and processing location volume plots - Unconstrained only, no total text

Generates single-panel combined processing location maps for:
  - BAU unconstrained (national)
  - Precursor unconstrained national
  - Precursor unconstrained regional

No "Total = X million tonnes" annotation is shown.
"""
import os
import sys
...
def main():
    ...
    # Only unconstrained scenarios, one per figure
    plot_descriptions = [
        {
            ...
            "years": [2040],
            "layers": ["bau_2040_mid_min_threshold_metal_tons"],
            "layers_names": ["BAU - No Environmental constraints"],
            "fig_suffix": "bau_2040_mid_country_unconstrained"
        },
        {
            "type": "final_stage_production_tons",
            ...
            "scenario_names": ["country"],
            "years": [2040],
            "layers": ["precursor_2040_mid_min_threshold_metal_tons"],
            "layers_names": ["Precursor - No Environmental constraints"],
            "fig_suffix": "precursor_2040_mid_country_unconstrained"
        },
        {
            "type": "final_stage_production_tons",
            "stage_type": ["Precursor related product"],
            ...
            "years": [2040],
            "layers": ["precursor_2040_mid_max_threshold_metal_tons"],
            "layers_names": ["Precursor - No Environmental constraints (Regional)"],
            "fig_suffix": "precursor_2040_mid_region_unconstrained"
        },
    ]
    ...
    for rt in result_type:
        for plot in plot_descriptions:
            ...
            # Single scenario → narrower figure
            figwidth = 8
                ...
                if sc_n == "key":
                    ...
                    for ky in range(len(keys)):
                        ...
                        if key == 'tonnage':
                            ...
                            yk = yl[0] + np.linspace(0.15 * dyl, 0.4 * dyl, Nk) + 0.4 * ky * dyl
                            yt = yk[-1] + np.diff(yk[-3:-1])
                            ax.text(xt, yt, 'Mineral produced', weight='bold',
                                    fontsize=textfontsize, ha='left', va='center')
                            for k in range(Nk):
                    ...
                    df.geometry.plot(
                        ...
                        alpha=0.7)
                    # NOTE: total text deliberately omitted

            fig_file = f"combined_processing_locations_maps_{fig_suffix}_nototal.png"
            plt.tight_layout()
